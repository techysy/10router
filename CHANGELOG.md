...
- **修复：侧边栏收起/展开按钮状态不翻转**：桌面端收起侧边栏后，Header 左上的按钮图标仍是 `menu_open`（收起箭头）——看不出当前状态、也没有"展开"暗示。现 `DashboardLayout` 把 `sidebarCollapsed` 传入 `Header`，展开态显示收起箭头、收起态显示汉堡图标，tooltip 同步翻转（新增词条 "Expand/Collapse sidebar"，zh-CN/zh-TW），并补 `aria-expanded`。
