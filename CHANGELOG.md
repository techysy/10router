...
- **CodeBuddy 国际版静态模型目录对齐服务端**：国际版（codebuddy.ai）按服务端目录重排——新增 Hy4-Preview / Hy3（走免费额度）、GPT-5.6 Sol/Terra/Luna、GPT-5.5、GPT-5.4、GPT-5.3-Codex、Gemini-3.5-Flash、GLM-5.3、Kimi-K3，移除已下线的 GLM-5.1/5.0、GLM-5v-Turbo、MiniMax-M3、Kimi-K2.7/K2.5，并为每个模型标注 `rateMultiplier` 积分倍率（0 = 免费额度）；配套 `tests/unit/codebuddy-intl-models.test.js` 断言由子集匹配改为全量精确列表。CN 站（copilot.tencent.com）目录未变。
