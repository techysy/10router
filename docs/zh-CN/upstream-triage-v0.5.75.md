...
| A8 | `eee3515e` | opencode-go：补新发布的模型 | ❌ 缺 **10** 个：`glm-5.3`、`kimi-k3`、`longcat-2.0`、`qwen3.8-max`、`qwen3.8-flash`、`hy4-preview`、`hy3`、`grok-4.6`、`muse-spark-1.2-contributor`、`muse-spark-1.3-contributor` | ✅ **已按活目录对齐（本提交），不照抄上游**：上游那份是 28 条时期的快照（且只用 `["openai"]` 单端点），而公开目录已是 **37** 条。已按公开目录补齐 18 条并为每条声明 `supportedFormats`（这才是关键：chatCore 只在模型声明了该 sourceFormat 时才用同格式传输）；`gpt-5.6-luna` 从 `/chat/completions` 纠正为 `/v1/responses`（上游同判，且我们此前那句 "responses 对此模型无效" 无来源）。同时收口 §3.2 的记账（2 条 canonical 行 + `omen-alpha` 进 allowlist）。数值校准（`glm-5.3` 1M、`qwen3.8-*` 新代际、`hy3*` 输出>窗口等）已在紧随其后的 A8b 提交完成（15 条 canonical 行，逐行标第一方来源） |
...
| 7 | **A8** opencode-go 按活目录对齐（补全 18 个，同时收口 §3.2 记账） | 本提交 | ✅ |
| 7b | **A8b** opencode-go 目录里能力数值/多模态按第一方重建（16 条 canonical 行） | 本提交 | ✅ |
