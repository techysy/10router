...
> **归档说明（2026-09-11）**：本文写于「1.0.9」尚在计划中时。**1.0.9 已作废**，其内容全部并入 **1.1.0**（见 `CHANGELOG.md` 的 `v1.1.0` 一节），因此本文更名为 `release-review-v1.1.0.md` 归位到 `docs/zh-CN/`；正文保留当时的 1.0.9 表述，以维持决策过程的原貌。
...
### 0.3 决定：**1.0.9 作废**（2026-09-11）

原计划的 1.0.9（那 10 个修复）**不再单独发版**，内容并入 1.1.0。

理由：1.0.8 的 tag 已被推到 `eeeb4b73`（越过了徽章），那 10 个提交落在 tag **之前**——
再为它们单独切一个 1.0.9，等于在同一段历史上又插一个 tag，只会让三份不一致的 1.0.8
产物之上再叠一层混乱；而它们的修复价值（`/v1/models` 500、拉取模型默认禁用、
CodeBuddy CN 目录对齐）由 1.1.0 一并交付即可，用户侧无感知差异。

**因此 1.1.0 = 自 1.0.8 tag 起的全部 42 个提交**（含原本属于 1.0.9 的那 10 个）。
下面 §1 的 1.0.9 清单仅作历史记录保留。

---

## 1. 两个版本各装什么

### 1.0.9（修复版，10 个提交）—— **已作废，见 §0.3**
