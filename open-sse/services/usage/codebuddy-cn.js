 ...
 * plus a built-in "总积分" aggregate row first, summing every pack's live
...
async function getCodeBuddyUsage(providerId, accessToken, apiKey, providerSpecificData, proxyOptions = null) {
  ...
  try {
    ...
    const quotas = {};
    // Built-in "总积分" aggregate, rendered first (above the Monthly row):
    // sums every pack's live balance — refills count their *Cycle* fields,
    // bonuses their lifetime *Capacity* fields, exactly the numbers the
    // per-pack rows below show — so users don't have to add packs up
    // themselves. resetAt stays null: the aggregate has no single cycle, its
