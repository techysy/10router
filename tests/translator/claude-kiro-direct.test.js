...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("carries the Claude system in the user turn, never as a top-level systemPrompt", () => {
    const out = C2K({
  ...
  it("keeps the user-turn system prefix stable across turns", () => {
    const credentials = {
      rawHeaders: { "x-session-id": "hermes-session-stable-system" },
      connectionId: "kiro-account-1",
    };
    const first = C2K({
    ...
    expect(systemPromptOf(first)).toContain("stable instruction");
